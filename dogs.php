...
<div id="filterForm">
    <form method="GET" action="">
        ...
        <select name="city" id="city">
            ...
            <?php
            $cityQuery = "SELECT DISTINCT city FROM animaux WHERE type=2 AND statue=2";
...
$sql = "SELECT * FROM animaux WHERE type=2 AND statue=2";
if ($cityFilter) {
    $sql .= " AND city = :city";
}
...
if ($stmt->rowCount() == 0) {
    ...
    echo "<div id='listdogs'>";
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo "<div class='dog'>" .
             "<img src='data:image/jpeg;base64," . base64_encode($row['image']) . "'/>" .
             "<div><h2>" . htmlspecialchars($row['nom']) . "</h2>" . 
             "<h2>Ville : " . htmlspecialchars($row['city']) . "</h2>" .
             "<a href='dogsprofile.php?id=" . htmlspecialchars($row['id']) . "'>Voir les détails</a>" .
             "</div></div>";
    }
    echo "</div>";
}
